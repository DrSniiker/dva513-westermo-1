...
# pip install pandas numpy scikit-learn tensorflow imbalanced-learn joblib
...
print(
    f"***************************Dataset: {Dataset} - will train {len(architectures)} models with different architectures..."
)
...
for i, arch in enumerate(architectures, 1):
    ...
    print(f" \n Max epochs: {epochs}, learning_rate: {learning_rate}, batch_size: {batch_size}")
    ...
    # The honest_ANN_mcc calculeted with the best threshold from the Val dataset
    # To compare the both, we shild use the honest MCC 
    ANN_mcc = matthews_corrcoef(y_test, y_pred)
    # *************************
    # The ANN_MCC calculeted with the best threshold from the test dataset
    y_val_probs = base_model.predict(x_val_scaled).ravel()
    val_mcc_scores = [
        matthews_corrcoef(y_val, (y_val_probs > t).astype(int))
        for t in thresholds
    ]
    best_threshold_val = thresholds[np.argmax(val_mcc_scores)]
    y_pred_val_mcc = (y_pred_probs > best_threshold_val).astype(int)
    honest_ANN_mcc = matthews_corrcoef(y_test, y_pred_val_mcc)
    ...
    conditions = [
    (y_test_arr == 1) & (y_pred_arr == 1),  # True Positive
    (y_test_arr == 0) & (y_pred_arr == 0),  # True Negative
    (y_test_arr == 0) & (y_pred_arr == 1),  # False Positive
    (y_test_arr == 1) & (y_pred_arr == 0)   # False Negative
    ]
    choices = ["TP", "TN", "FP", "FN"]

    conf_df = pd.DataFrame({
        "attack_type": df_test["class"].values,
        "classification_result": np.select(conditions, choices, default="unknown")
    })
    type_counts_df = pd.crosstab(conf_df["attack_type"], conf_df["classification_result"])

    print("\n--- Classification Breakdown per Attack Type ---")
    ...
    print(classification_report(y_test_arr, y_pred_arr,
                                ...
                                digits=4))
    print(f"  Global MCC (threshold={best_threshold:.4f}): {ANN_mcc:.4f}, (best_threshold_val={best_threshold_val:.4f}): Honest_MCC: {honest_ANN_mcc:.4f}")
    ...
    for label, row in type_counts_df.sort_index().iterrows():
        ...
        print(f"  {label:<20} {tp_:>7} {tn_:>7} {fp_:>7} {fn_:>7}"
            f"  {metric_name}={metric_val:.1%}  Archi-MCC={ANN_mcc:.3f} Honest-MCC={honest_ANN_mcc:.3f} Acc={acc_:.3f}")
